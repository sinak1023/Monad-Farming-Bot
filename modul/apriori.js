...
async function stakeMON(wallet, cycleNumber) {
  try {
    ...
    console.log(`🔄 Staking: ${ethers.utils.formatEther(stakeAmount)} MON`.green);
    ...
    console.log(`✅ Initiating Stake`.magenta);
...
async function requestUnstakeAprMON(wallet, amountToUnstake, cycleNumber) {
  try {
    console.log(`🔄 Unstaking: ${ethers.utils.formatEther(amountToUnstake)} aprMON`.green);
    ...
    console.log(`🔄 Initiating Unstake`.magenta);
    const txResponse = await wallet.sendTransaction(tx);
    console.log(`➡️ Transaction Hash: ${txResponse.hash}`.yellow);
...
async function checkClaimableStatus(walletAddress) {
  try {
    ...
    if (claimableRequest) {
      console.log(`✅ Found claimable request: ${claimableRequest.id}`.green);
      return {
    ...
    console.error(`❌ Failed to check claimable status:`.red, error.message);
    return {
      id: null,
      isClaimable: false,
    };
...
async function claimMON(wallet, cycleNumber) {
  try {
    ...
    if (!isClaimable || !id) {
      console.log(`❌ No claimable requests found`.red);
      return null;
    }

    console.log(`✅ Claiming withdrawal: ${id}`.green);
    ...
    console.log(`✅ Initiating Claim`.green);
    const txResponse = await wallet.sendTransaction(tx);
    console.log(`➡️ Transaction Hash: ${txResponse.hash}`.grey);
...
async function runCycle(wallet, cycleNumber) {
  try {
    ...
    console.log(`⏳ Waiting ${delayTimeBeforeUnstake / 1000} seconds`.grey);
    await delay(delayTimeBeforeUnstake);

    await requestUnstakeAprMON(wallet, stakeAmount, cycleNumber);

    console.log(`✅ Waiting for claim eligibility`.green);
    await delay(660000);
...
async function main() {
  try {
    ...
    console.log(`🚀 Starting Apriori Module 🚀`.blue);
