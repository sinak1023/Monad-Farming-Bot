...
async function connectToRpc() {
    for (const url of RPC_URLS) {
        try {
            ...
            console.log(`🚀 Starting BeanSwap Module 🚀`.blue);
            return provider;
        } catch (error) {
...
async function swapEthForTokens(wallet, tokenAddress, amountInWei, tokenSymbol) {
    ...
    try {
        console.log(`🔄 Swapping ${ethers.utils.formatEther(amountInWei)} MON to ${tokenSymbol}`.green);
...
async function swapTokensForEth(wallet, tokenAddress, tokenSymbol) {
    ...
    if (balance.eq(0)) {
        console.log(`❌ No ${tokenSymbol} balance, skipping`.black);
        return;
    }
    ...
    try {
        console.log(`🔄 Swapping ${tokenSymbol} to MON`.green);
